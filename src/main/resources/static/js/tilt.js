var PhotoTilt = function(options) {
	...
	var updatePosition = function() {
		...
		var pxToMoveAmount = (centerOffset + pxToMove) * -1;
		updateImgPosition(imgData,  pxToMoveAmount);
		updateTiltBar(tilt);
			...
			window.addEventListener('deviceorientation', function(eventData) {
			  ...
			  if(skippedFrames < 1){
			    ...
			    skippedFrames = 0;
  				...
  				if (!disableTilt && $('.mask').hasClass('active')) {
	...
	var setTranslateX = function(node, amount) {
		...
		var duration = 0.5;
		var posDifference;
		if(lastTranslatePos != null){
			posDifference = Math.abs(amount-lastTranslatePos);
			duration = 0.6 + (0.9 * (posDifference/2000));
		}
		
		$('.mask img').css('transition', 'transform ' + duration + 's linear')
		lastTranslatePos = Math.round(amount);
		lastPosDifference = posDifference;
